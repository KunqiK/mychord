...
1. **无根 voicing 候选生成**: 对 {Ab,C,Eb} 也生成 F 根 (Fm7 缺根读法) — jazz shell voicing 是常态, 需要低音轨迹/调性做判别
2. **循环传播**: 这首歌 30 次 Fm7/Bb, 证据完整的实例标对后按 pc 轮廓相似度传播给证据残缺的实例 (EDM 循环结构通用)
3. BTC large-voca 第二意见 (MIT, 12MB, 库里现成) 边界求交 + 根音投票
4. 钢琴转录换 transkun A/B (MAESTRO F1 0.93 vs ByteDance 0.82) — 中声部召回可能是它的强项
