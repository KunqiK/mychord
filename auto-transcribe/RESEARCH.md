...
**Claude 符号裁决器: 已实测证伪于 root (08-02)**
- 实现 referee.py (claude -p headless, sonnet): 只裁「conf<0.6 ∨ BTC 根音异议」段, 只准选既有候选, 只改标签不动时间轴; 31 首 ~340 处改动, 断点续跑+超时跳过
- **A/B (卡尺 v2.1 同集): root 均值 46.3→45.3 (−1.0pp), 中位 −0.3; 兼容中位 50.8→53.4 (+2.6)** — LLM 让读法更"顺耳"但根音改错多于改对, 与 GPT-judge 文献 (错误判别仅 64.8%) 和 Chordonomicon 先验的败因同构: **本曲库爱非常规进行, 任何"陈词滥调先验"都会拖 root**。user 第一指标是 root → 不并入管线, referee.py 留档
- 未试变体 (记录不追): 更强模型 (fable/opus)、更严门控 (仅 BTC+模板同时低置信)、只裁质性不裁根音
- 至此路线图 12 方案全部落地或证伪。存活: BTC 双权重打分 (root 决策主引擎) + 移调增强 + HookTheory 数据 + 全自动收丹链 + 卡尺 v2; 证伪: NNLS×2 / 进行先验×2 / LLM裁决×1 / 本地7B / 音频域重合成 (调查期)。**主线明确 = 数据飞轮一条路: 更多 HookTheory + 更多 user 修正 → 重训 → harvest**
- 下载 2 轮后 1084 首在手 (损耗 = 死链 + bot-check 复发); 满员后 --features → run5 → harvest run5

**run5: 更多数据首次出现回退 (08-03 晨) — 数据质量开始压过数量**
- run5 (32 用户×2 + 1085 HT = 15,797 窗口): 早停首次生效 (36 轮后收炉, 省 14 轮); **最佳 val 32.09% < run4 的 33.5% → 不部署, best.pt 保持 run4-ep37**
- 解读: 第二批 clips (按标注秒数排名 300-2000) 标注更薄/对齐更糙, 稀释了信号; 也进一步冲淡用户域占比 (15%→11%)
- harvest.py 补守卫: 新最佳 ≤ 现任 (best.pt 内存的 val_acc) 时只归档不部署不扫票权 — 防盲目换代
- run6 方向 (待做): 按质量过滤 (ann_sec 下限 / 只要 REFINED_ALIGNMENT 高置信) 而非按量; 或 USER_REPEAT 调 3-4 抬用户域占比; bot-check 647 首等长冷却补下

**run6: 用户域加权 ×4 也证伪 — 病根锁定在那 362 首增量本身 (08-04 晨)**
- run6 (1085 HT + 用户×4 = 17,689 窗口, 21 分/轮, 早停于 39 轮): **val 31.74% @ep26 < 现任 33.5% → harvest 守卫自动拒绝部署** (守卫首次实战生效, 且跳过票权扫描省了半小时)
- **判决性对照**: run5 (1085, 用户×2) 32.09 / run6 (1085, 用户×4) 31.74 — 加权不但没救回来, 反而更差 → **"用户域被稀释"的诊断被证伪**; 唯一与 run4 (723, 33.5) 不同的变量是**那 362 首增量数据本身有毒**
- ann_sec 分布已排除"标注太薄" (1084/1085 都 ≥60s)。剩下的头号嫌疑: **音频与标注不是同一版本** — HookTheory 标注绑定特定录音, yt-dlp 抓到翻唱/remix/变速/live 版就是纯噪声标签; 第二批下载正是 bot-check 高发批次, 存活样本可能系统性偏"冷门/易错配"
- 下一步不再盲训: 先做**自洽性核验** (用现役 best.pt 对每首 HT clip 推理, 与其标签算逐帧一致率, 接近随机的判为错配) → 剔除后再训 run7。**规矩升级: 数据入库前先验对齐, 不验就是在往炉子里扔杂质**

**核验结果: 错配假设也证伪, 真凶是「域太远 + 数据太易」(08-04 晨, verify_ht.py)**
- 方法论陷阱先踩后修: 用现役 best.pt 当裁判量出 0.86 中位一致率 — 但它**训练时见过这批 clip**, 量的是背诵不是对齐。换成官方出厂权重 (对我方全部数据都是盲的) 才是无偏裁判
- 无偏裁判结果: **HookTheory clip root 一致率中位 0.839, 99% 高于任何合理门槛, 无近随机尾巴 → 音频与标注对得很好, 「抓错版本」假设证伪**
- **判决性对比 (同一裁判, 两个语料)**: HookTheory 中位 **0.839** vs 用户曲库 中位 **0.484** — 35 个点的鸿沟。含义: HookTheory 是**西方流行, 对预训练模型本来就简单**, 里面可榨的信号早被 run4 的 723 首吃干净; 再加只是把模型往流行域拽, 离用户的日系电子更远。**"更多 HookTheory" 已到边际收益为负的拐点**
- 结论 (三连否定后的正路): 不再堆 HT 数量。两条候选 —— ①**难例挑选**: 用同一裁判给 1085 首打难度分, 只留裁判做得最差的那批 (和声最密/最含糊 = 最接近用户域, 信号最浓), 用户歌 ×2 训 run7; ②**扩用户域标注** (真正的瓶颈, 靠用户修正回流的数据飞轮)
- 全量难度分计算完毕 (features_ht_agree.json, 1085 首)

**结案: HookTheory 微调这条轴已耗尽 — 四个权重端到端完全打平 (08-04)**
- run7 (难例挑选, 一致率<0.75 → 330 首, 6021 窗口): val 29.72 @ep16, 早停于 29 轮 — **难例假设证伪** (同量级下 run3 的 246 首"简单"clip 反而拿 31.56)
- **先修方法论再下结论**: 实测 val_acc 相邻轮平均跳动 **1.24pp** (6 首/92 窗口太小), 且"取最好一轮"是 50 次噪声抽样的最大值 = 有偏, 跑得久的占便宜。按 20 轮后中位重排: **run6 30.6 > run4 30.3 > run5 29.8** → **先前报告的 run5/run6"退步"实为统计打平, 是我的判据错了**
- **判决性实验 (compare_ckpts.py, 换权重跑 34 首端到端, 票权固定)**:

  | 权重 | root 均值 | root 中位 | 兼容 |
  |---|---|---|---|
  | run4 (现役) | 46.3% | 49.1% | 50.5% |
  | run5 | 46.2% | **50.2%** | 50.2% |
  | run7 | 46.2% | **50.3%** | 50.1% |
  | run6 | 45.7% | 48.5% | 50.4% |

- **结论: 四个模型端到端等价 (均值极差 0.6pp)。val_acc 上 3.8pp 的差距 (33.5↔29.7) 到了管线出口只剩 0.6pp** — BTC 后验对本管线的贡献已饱和, 换哪个都一样。**继续在 HookTheory 上微调不再是杠杆, 该轴结案**。现役保持 run4 (无证据支持换代, 稳定性有价值)
- 附带教训: run7 的 val 最差 (29.7) 却拿到最好的 root 中位 (50.3) — 再次证明小验证集的 val_acc 不能当选型依据。**规矩: 选型一律走 compare_ckpts.py**
- 剩余真杠杆只剩一条: **用户域标注数据** (用户修正回流)。管线侧另需结构性新想法, 而非同一轴上继续调参
